...
class Action:
    ...
    letter_codes = [ord(ch) for ch in 'WASDRQwasdrq'] + [curses.KEY_UP, curses.KEY_LEFT, curses.KEY_DOWN,
                                                         curses.KEY_RIGHT]
...
class Screen:
    ...
    def draw(self, states):
        self.stdscr.clear()
        self.stdscr.addstr('Score:' + str(self.grid.score) + "\n")
        x = 8  # 小方格宽度，单位为字符
        y = 4  # 小方格高度，单位为字符
        crosschar = '+'  # 交叉点
        xchar = '-'  # 横线
            ...
            for i in range(self.grid.width * x + 1):
                ...
                if i % x == 0 and j % y == 0:
                    ...
                    if self.grid.field[row][col] != 0:
                        if (row, col) in self.grid.newfield:
                            self.grid.newfield.remove((row,col))
        ...
        if states == GameManager.GAMEOVER:
            self.stdscr.addstr(self.GAME_OVER_INFO)
        else:
            self.stdscr.addstr(self.INFOR)
        # stdscr.getch()
